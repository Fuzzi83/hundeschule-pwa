...
// Umsatz/Offene über customer.cards
// - Umsatz: method=cash oder (method=invoice && invoiceIssued=true)
// - Offene Karten: method=invoice && invoiceIssued=false

import { esc } from "../core/utils.js";
import { customers, visits, stays, getCustomerById } from "../core/storage.js";

/* ------------------------------------------------------------------ */
/* Navigation Helper – zu Kunden springen                             */
/* ------------------------------------------------------------------ */
...
export function renderReports() {
  ...
  V.forEach((v) => {
    ...
    html += `
      <tr>
        ...
        <td>${v.method === "ten" ? "Karte" : (v.method === "cash" ? "Bar" : "Rechnung")}</td>
  ...
  // Offene Karten
  const openCards = customers.flatMap((c) =>
    (c.cards || [])
      .filter(
        (k) =>
          k.method === "invoice" &&
          !k.invoiceIssued &&
          inRange(String(k.date || "").slice(0, 10))
      )
      .map((k) => ({ price: Number(k.pricePaid || 0) }))
  );

  const sumCards = openCards.reduce((a, x) => a + x.price, 0);

  document.getElementById("r_summary").innerHTML =
    `Offene Posten (Tag): ${sumVisit.toFixed(2)} € · ` +
    `Offene Karten: ${sumCards.toFixed(2)} €`;
}
...
export function renderRevenueAll() {
  const type = document.getElementById("rev_type")?.value || "all";
  ...
  customers.forEach((c) => {
    (c.cards || [])
      .filter(
        (k) =>
          k.method === "cash" ||
          (k.method === "invoice" && !!k.invoiceIssued)
      )
      .filter((k) => inRange(String(k.date || "").slice(0, 10)))
      .forEach((k) => {
  ...
  visits
    .filter(
      (v) => v.payStatus === "paid" && v.method !== "ten" && inRange(v.date)
    )
    .forEach((v) => {
  ...
  stays
    .filter(
      (s) =>
        s.payStatus === "paid" &&
        inRange(
          ((s.checkoutDate || s.checkinAt || "") + "").slice(0, 10)
        )
    )
    .forEach((s) => {
      if (type === "all" || type === "payments") {
        ...
        rows.push({
          date: ((s.checkoutDate || s.checkinAt || "") + "").slice(
            0,
            10
          ),
          owner: c?.name || "",
  ...
  document.getElementById("rev_summary").textContent =
    `Summe: ${sum.toFixed(2)} €`;
}

/* ------------------------------------------------------------------ */
/* Konflikte (unverändert zum alten System)                            */
/* ------------------------------------------------------------------ */
...
export function renderConflictReport() {
  ...
  if (C.length === 0) {
    wrap.innerHTML = `<div class="muted">Keine Konflikte.</div>`;
    return;
  }
