...
// Vollständige, stabile Version (2026-02-17)

const SECTION_PREFIX = "page_";
const HIDDEN_CLASS = "hidden";

// Aliases -> erlaubt alte Bezeichner
...
function discoverSections() {
    const sections = {};
    document.querySelectorAll(`.page[id^="${SECTION_PREFIX}"]`)
        .forEach(el => {
            const name = el.id.substring(SECTION_PREFIX.length)
                .trim()
                .toLowerCase();
            sections[name] = el;
        });
...
export function makeGo() {
    ...
    function ensureInit() {
        if (!state.sections) {
            state.sections = discoverSections();
        }
    }
...
// ----------------------------
// Bindet alle Buttons mit data-target
// ----------------------------
